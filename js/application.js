...
Application.prototype.dbPurgeOld = function () {
    this.localDb.transaction(
        function(transaction) {
            transaction.executeSql('DELETE FROM News ORDER BY date DESC LIMIT 30, 1000');
            transaction.executeSql('DELETE FROM FloorUpdates ORDER BY date DESC LIMIT 30, 1000');
            transaction.executeSql('DELETE FROM LeadershipNotices ORDER BY date DESC LIMIT 30, 1000');
            transaction.executeSql('DELETE FROM Hearings ORDER BY date DESC LIMIT 30, 1000');
            transaction.executeSql('DELETE FROM LastUpdate ORDER BY date DESC LIMIT 30, 1000');
            transaction.executeSql('DELETE FROM Documents ORDER BY date DESC LIMIT 30, 1000');
        }
    );    
}
