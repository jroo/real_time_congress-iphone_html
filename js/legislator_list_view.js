...
function LegislatorListView() {
    ...
    self.localToList = function(results) {
        ...
        for (var i=0; i<results.rows.length; i++) {
            ...
            latest_list.push({row_type:'content', id:row.id, title:row.title, firstname:firstname, lastname:row.lastname});
        }
        return latest_list;
    }
    ...
    self.renderRow = function(row, dest_list) {
    	...
    	$(anchor).click(function() {
            self.loadLegislator(row.bioguide_id, row.title + '. ' + row.firstname + ' ' + row.lastname);
    	});
